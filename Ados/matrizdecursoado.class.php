...
class MatrizCursoAdo extends ADO {
    ...
    public function insereObjeto(\Model $objetoModelo) {
        $query = "insert into Matrizes_de_cursos values ";
        foreach ($objetoModelo->getMatrzDiscCodigo() as $disc) {
            $query .= "({$objetoModelo->getMatrzCursId()}, {$disc}),";
        }
        $size = strlen($query);
